...
{
	return [[NSBundle bundleWithPath:appPath] objectForInfoDictionaryKey:key];
}
...
{
	if (SUForeignInfoValueForKey(appPath, @"CFBundleName")) { return SUForeignInfoValueForKey(appPath, @"CFBundleName"); }
	return [[[NSFileManager defaultManager] displayNameAtPath:[[NSBundle bundleWithPath:appPath] bundlePath]] stringByDeletingPathExtension];
}
...
{
	if (SUForeignInfoValueForKey(appPath, @"CFBundleDisplayName")) { return SUForeignInfoValueForKey(appPath, @"CFBundleDisplayName"); }
...
{
	return SUForeignInfoValueForKey(appPath, @"CFBundleVersion");
}
...
{
	NSString *shortVersionString = SUForeignInfoValueForKey(appPath, @"CFBundleShortVersionString");
	if (shortVersionString)
	{
		if (![shortVersionString isEqualToString:SUForeignAppVersion(appPath)])
			shortVersionString = [shortVersionString stringByAppendingFormat:@"/%@", SUForeignAppVersion(appPath)];
		return shortVersionString;
	}
	else
		return SUForeignAppVersion(appPath); // fall back on CFBundleVersion
}
